...
function parseSoapResponseRobust(rawXml: string): TicimaxSiparis[] {
    ...
    while ((match = siparisRegex.exec(cleanXml)) !== null) {
        ...
        // Text-to-ID Mapping Check
        if (isNaN(siparisDurumu)) {
            // Ticimax bazen "Teslim Edildi" gibi text dönüyor. Bunu ID'ye çevirelim.
            const statusMap: Record<string, number> = {
                ...
                "İptal Edildi": 8, "İade Edildi": 9, "Silinmiş": 10, "İade Talebi Alındı": 11,
                "İade Ulaştı Ödeme Yapılacak": 12, "İade Ödemesi Yapıldı": 13,
                "Teslimat Öncesi İptal Talebi": 14, "İptal Talebi": 15,
                "Kısmi İade Talebi": 16, "Kısmi İade Yapıldı": 17
            };
            // Case-insensitive check
            const matchedKey = Object.keys(statusMap).find(k => k.toLowerCase() === rawStatus.toLowerCase());
            if (matchedKey) {
                siparisDurumu = statusMap[matchedKey];
            } else {
                siparisDurumu = -1;
            }
        }
